...
**P1-3. 每日彙整 marker 寫入順序 — 已與使用者確認，維持現狀**

`_check_daily_summary()`（`kepware_log_service.py:300`）為「先寄信、成功後寫 marker」。若寄信成功但寫 marker 前 crash，隔天會重寄一次昨天日報。使用者於 2026-07-06 確認**維持現狀**（極罕見的當機時間點才會重複寄信，可接受；優先避免「以為寄過了但其實沒寄到」的漏寄風險）。**此項已結案，後續模型不需再改動這段邏輯。**
...
P1-3 已於 2026-07-06 由使用者決策結案（維持現狀），不在後續路線圖中。
